...
def build_system_prompt(state: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"- {_master_label()}의 지인, 동료, 거래처 등 인간관계가 있는 상대: '{_caller_intro()}'로 소개한다.\n\n"
        "## 대화 흐름\n"
...
def twiml_start(call_id: str) -> PlainTextResponse:
    ...
    vr = VoiceResponse()
    opening_text = f"안녕하세요. {intro}. {task_text}"
    ...
    if opening_tts_url:
        ...
        vr.say(f"안녕하세요. {_caller_intro()}.", language="ko-KR", voice="alice")
        vr.pause(length=1)
...
async def internal_create(call_id: str, target_name: str = Form(default="상대방"), task: str = Form(default="")) -> dict[str, Any]:
    ...
    for key, text in [
        ...
        ("fast_end", f"감사합니다. {_master_label()}께 바로 전달하겠습니다. 오늘 좋은 하루 보내세요."),
        ("fast_end_alt", f"말씀 감사합니다. 전달받은 내용은 {_master_label()}께 바로 공유드리겠습니다. 오늘 좋은 하루 보내세요."),
    ]:
